...
describe("ProviderTransform.message - DeepSeek reasoning content", () => {
  const interleavedModel = (overrides: Partial<any> = {}) =>
    ({
  ...
  test("interleaved assistant messages keep existing reasoning_content when no reasoning part is present", () => {
    const msgs = [
      {
        role: "assistant",
    ...
    const result = ProviderTransform.message(msgs, interleavedModel(), {}) as any[]

    expect(result[0].content).toEqual([{ type: "text", text: "Answer" }])
    expect(result[0].providerOptions?.openaiCompatible?.reasoning_content).toBe("already captured")
  })

  test("legacy string assistant messages include empty reasoning_content for interleaved models", () => {
